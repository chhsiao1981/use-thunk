...
export type doModule<S extends State, T extends ThunkModule<S>> = {
  ...
  [action in keyof toThunkFuncMap<T>]: VoidReturnType<
    toThunkFuncMap<T>[action]
  >;
} & Omit<defaultDoModule, keyof toThunkFuncMap<T>>;

type defaultDoModule = {
  [action in keyof defaultThunkFuncMap]: VoidReturnType<
    defaultThunkFuncMap[action]
  >;
};
...
export const constructDoModule = <S extends State, T extends ThunkModule<S>>(
  ...
  Object.keys(DEFAULT_THUNK_FUNC_MAP).reduce((val, eachAction) => {
    ...
    const action =
      DEFAULT_THUNK_FUNC_MAP[eachAction as keyof defaultThunkFuncMap];
 ...
 * @returns doModule
 */
export const doMod = <S extends State, T extends ThunkModule<S>>(
  moduleName: string,
) => {
